...
const Button = ({ colors, label, color, onPress, btnStyle, textStyle, leftIcon, iconStyle, absolute, loading, disable, border }) => {
    return (
        ...
        <TouchableOpacity disabled={disable} onPress={onPress} style={[styles.container, border && styles.border, absolute && { position: 'absolute', width: wp(90), alignSelf: 'center', bottom: wp(4) }, disable && { opacity: 0.5 }, btnStyle && btnStyle,]} >
            {leftIcon &&
                <View>
                    <Image source={leftIcon} style={[styles.icon, iconStyle && iconStyle]} />
                    <Spacer row={wp(5)} />
                </View>
            }
            {loading ?
